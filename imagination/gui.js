...
imagination.gui.imaginationAuthGUI = async function(fileEls = [], startLogonProcess = new Function(), prompt = (()=>"42"), endLogonProcess = new Function(), storage = localStorage) {
    ...
    if (!result.ok) return endLogonProcess("Authentication failed: " + result.status + " " + result.statusText + "\n" + await result.text())
    if (privkey_data.startsWith("encrypted:")) {
        let password = await prompt("Enter your secret quack, then press Enter:");
        try {
    ...
    try {
        ...
        document.cookie = "token=" + encodeURIComponent(token) + "; expires=" + new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toUTCString() + (location.protocol == "https:" ? "; secure" : "");;
        location.reload();
    } catch (e) {
...
imagination.gui.imaginationSavedEncrypt = async function(fileEls = [], startLogonProcess = new Function(), prompt = (()=>"42"), confirm = (()=>true), endLogonProcess = new Function(), storage = localStorage) {
    ...
    if (encryptMark) {
        if (confirm("The key is already encrypted. Would you like to decrypt the key?")) {
            let password = prompt("Enter your secret quack, then press Enter:");
            try {
                storage.setItem("privk", await imagination.encryption.decryptAES(JSON.parse(privkey_data.replace("encrypted:", "")), password));
            } catch (e) {
                return endLogonProcess("Seems like your secret quack was not correct. Try again later.\n\n-----BEGIN TECHNICAL INFO-----\n" +  + e.toString() + "\n-----END TECHNICAL INFO-----");
            }
        }
    } else {
        let password = prompt("Enter a new secret quack, then press Enter:");
        try {
